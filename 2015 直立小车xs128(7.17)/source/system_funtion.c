...
void init_pwm(void) 
{
  ...
  PWMPER01  =  4000;   //通道1的频率为20KHz,用于电机的驱动  A
  PWMPER23  =  4000;   //通道3的频率为20KHz,用于电机的驱动  B
...
void INIT_AD(void) 
{
     ...
     ATD0CTL1 = 0x40; // 12-bit data                   
     ATD0CTL2 = 0x40;       // 快速清除所有标志位                    
 /*   
  ATD0CTL0=0X03;      //AN3为回环终点
    ATD0CTL1=0X40;      //12位精度，采样前不放电，外部触发端口为0
    ATD0CTL2=0X40;      //快速清零，禁止外部触发，禁止中断
    ATD0CTL3=0XA8;      //1010 1000,数据右对齐，序列长度为4，无FIFO，断点时完成当前转换然后冻结
    ATD0CTL4=0X04;      //0001 1111 采样时间为4个AD周期
                        //fATDCLK=fBUS/(2*(PRS+1)),PRS=3,fBUS=64MHz,fATDCLK=8MHz                        
    ATD0CTL5=0X30;      //非特殊通道采样；连续采样模式，多通道采样，第一个采样为AN0 
*/
